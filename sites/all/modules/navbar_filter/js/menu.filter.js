  ...
  Drupal.behaviors.navbar_filter = {
    attach: function () {
      $('.navbar-filter').keyup(function () {
        ...
        var menu_ul = $(this).parent('.form-item').siblings('.navbar-menu');
        ...
        if (thisvalue.length === 0) {
          ...
          menu_ul.find('a').each(function () {
            ...
            if (match !== null && match.length > 0) {
              $(this).show();
              $(this).parents('li.navbar-level-1').find('a').eq(0).show();
              $(this).parents('li.navbar-level-2').find('a').eq(0).show();
              $(this).parents('li.navbar-level-1').addClass('open');
              $(this).parents('li.navbar-level-2').addClass('open');
            }
            else {
              $(this).hide();
            }
